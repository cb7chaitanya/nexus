...
describe("knowledge base routes", () => {
  ...
  it("presigns a document upload, creating a PENDING_UPLOAD document row", async () => {
    ...
    // Presigned PUT (not POST — see lib/storage.ts's createPresignedUpload,
    // switched because R2 doesn't support presigned POST at all): the
    // storage key is embedded directly in the signed URL's path, no
    // separate form fields.
    expect(typeof body.uploadUrl).toBe("string");
    expect(body.uploadUrl).toContain(body.document.storageKey);
    expect(body.uploadUrl).toContain("X-Amz-Signature=");
    expect(body.uploadFields).toBeUndefined();
